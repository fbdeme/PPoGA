            ...
            for rela, e_list in r_e_dict.items():
                ...
                for ent in e_list:
                    if entid_name[ent] not in name_dict[entid_name[topic_e]][h_t][rela]:
                        name_dict[entid_name[topic_e]][h_t][rela].append(
                            entid_name[ent]
                        )
    return name_dict
...
def if_finish_list(
    ...
    for dep, ent_rel_ent_dict in depth_ent_rel_ent_dict.items():
        for topic_e, h_t_dict in ent_rel_ent_dict.items():
            ...
            for h_t, r_e_dict in h_t_dict.items():
                for rela, e_list in r_e_dict.items():
                    if (
                        all(entid_name[item].startswith("m.") for item in e_list)
                        and len(e_list) > 10
                    ):
    ...
    if flag:
        ...
        print(
            "filter already",
            [entid_name[ent_i] for ent_i in new_lst],
            [entid_name[ent_i] for ent_i in all_ent_set],
            other_entities_name,
        )
        ...
        add_ent_list = [
            name_entid[ent_i] for ent_i in add_ent_list if ent_i in other_entities_name
        ]
