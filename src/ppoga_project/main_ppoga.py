...
Usage:
    python main_ppoga.py --question "Who directed The Godfather?" --max_iterations 8
    python main_ppoga.py --dataset cwq --max_iterations 10
"""
...
from ppoga.predictive_planner import PredictivePlanner
from ppoga.enhanced_executor import EnhancedExecutor
from ppoga.enhanced_memory import PPoGAMemory
...
def run_ppoga(
    ...
    """
    Run PPoGA system combining PoG's SPARQL engine with predictive planning
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"🎉 PPoGA EXECUTION COMPLETED")
        print(f"{'='*60}")
        ...
        print(f"❌ PPoGA execution failed: {e}")
        return {
            "success": False,
            "question": question,
            "error": str(e),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PPoGA: Predictive Plan-on-Graph with Action"
    )
    ...
    if args.question:
        ...
        print(f"Running PPoGA for question: {args.question}")

        # Load topic entities (placeholder)
        topic_entities = load_sample_entities(args.question)

        # Run PPoGA
        result = run_ppoga(args.question, topic_entities, config)

        # Save results
        timestamp = int(time.time())
        output_file = os.path.join(config.output_dir, f"ppoga_result_{timestamp}.json")
        ...
        print("Example: python main_ppoga.py --question 'Who directed The Godfather?'")
